...
#Matrix
FreeThrowsAttempt <- rbind(
  KobeBryant_FTA, JoeJohnson_FTA, LeBronJames_FTA,
...
rownames(df) <- NULL
df
df <- melt(df, id.vars = 'Year', variable.name = 'players')
df
ggplot(df, aes(Year, value, colour=players)) +
  geom_point() +
